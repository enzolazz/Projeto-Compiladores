...
#define db_print(...) for (const auto& a : {__VA_ARGS__}) std::cout << a; (std::cout << std::endl)

// Print a message and variables to stderr
#define db_msg(msg, ...) __debug::varsNamesPrefix("(Debug)", msg __VA_OPT__(, #__VA_ARGS__, ) __VA_ARGS__)
...
void varsNames(std::size_t i, const std::vector<std::string>& names, T v, Var... vars) {
    std::cerr << std::boolalpha << names[i] << '=' << v << ",, ";
    varsNames(i + 1, names, vars...);
}

template <typename... Var>
void varsNamesPrefix(const char*&& prefix, const char*&& msg, const char*&& names, Var... vars) {
